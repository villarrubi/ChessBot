...
def main() -> None:
    ...
    parser.add_argument("--openings", type=Path, default=ROOT / "data" / "openings" / "strength-v1.json")
    parser.add_argument("--levels", default="1320,1600,2000,2400,2800,3100",
                        help="comma-separated Stockfish UCI_Elo levels")
    parser.add_argument("--games-per-level", type=int, default=16)
    ...
    if args.depth < 1 or args.max_plies < 1:
        parser.error("depth and max-plies must be positive")
    try:
    ...
    args.openings.resolve(strict=True)
