...
## Evaluar jugadas propuestas y planes

Escribe una pregunta y, si quieres comparar otras jugadas, indícalas en **Alternativas** con notación SAN (`Qxc3, Qxf1`) o UCI (`d3c3, d3f1`), separadas por comas, espacios, punto y coma o `/`. Se aceptan hasta cuatro y también se detectan jugadas SAN/UCI escritas dentro de la pregunta. Todas se validan en la posición anterior a la fila seleccionada. Desde el escritorio se recalculan juntas con la mejor referencia mediante una búsqueda MultiPV restringida con el historial, opciones y límite originales; así sus puntuaciones son comparables porque proceden de la misma búsqueda. Por ello, proponer jugadas a profundidad 20 puede volver a requerir un cálculo largo.

Cada propuesta llega al explicador con su evaluación, diferencia respecto a la mejor jugada, PV, posición resultante y señales verificables de capturas, jaques y continuaciones de ambos bandos. La respuesta da un veredicto individual y muestra para cada propuesta la respuesta principal, la continuación y las maniobras observables de ambos jugadores. Después, la IA formula planes condicionales y riesgos generales. Una idea estratégica sigue siendo interpretación del modelo: las cifras y variantes de **Cálculos del motor** son la evidencia comprobable.

Cada pregunta es independiente. No hay memoria conversacional ni edición libre del tablero. Para una continuación que no pueda expresarse como una jugada legal desde la posición seleccionada, analiza otra FEN.
