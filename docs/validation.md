...
## Análisis interactivo y correcciones de búsqueda (22 de septiembre de 2026)

El modo de escritorio publica todas las jugadas al iniciar, abre la última y presenta iteraciones parciales. Las pruebas de integración verifican dos cambios de prioridad antes de terminar profundidad 20, parada/cancelación conservando resultados, PV legales con sus FEN, varias partidas, búsqueda de la jugada realizada fuera de MultiPV y exportaciones PGN/FEN, incluida una posición terminal. WinForms verifica filas navegables mientras calcula, resaltado de última jugada y anterior, recorrido de variantes, retorno al tablero, cancelación y bloqueo de explicaciones basadas en filas incompletas. El panel derecho permite desplazarse en ventanas pequeñas para que desplegar el gráfico no oculte definitivamente los cálculos.

Regresión concreta corregida: con FEN `7k/5K2/8/6Q1/8/8/8/8 w - - 0 1`, `go depth 1 searchmoves g5g6` producía **+932 cp** en `Optimized` aunque `Qg6` deja al rival ahogado. La prueba nueva exige **0 cp** a profundidades 1, 2 y 4. También contrasta la detección temprana de jugadas legales con la generación completa en posiciones de mate, ahogado, en passant clavado y recorridos reproducibles, conservando FEN y hash. La compilación final pasa **46 casos C++ y más de 30.300 aserciones**.

Una primera variante añadió además futilidad restringida a ventana estrecha y comprobación legal anticipada en todos los nodos. Obtuvo 7 victorias, 42 tablas y 15 derrotas (**28–36**) contra `56e8056` en 64 partidas, 50 ms/jugada, un hilo, aperturas con colores intercambiados y máximo 120 plies. No se conserva esa variante: se retiró el cambio de futilidad y la comprobación adicional se hace al devolver un corte estático/null-move, además de quietud. Se mantienen la corrección de ahogado y las protecciones de jaques/promociones/ventanas de mate. El resultado desfavorable forma parte de la evidencia, no se interpreta como mejora de fuerza.

La segunda variante obtuvo **12 victorias, 37 tablas y 15 derrotas (30,5–33,5)** con el mismo protocolo contra `56e8056`, ambos en `Optimized`, evaluación manual, Hash predeterminado de 16 MB y sin libro. La aproximación por parejas produjo un intervalo del 95 % para la puntuación entre 40,9 % y 54,4 %: no demuestra una mejora de fuerza ni descarta una regresión pequeña. Además, las muestras de profundidad 20/MultiPV 3/8 hilos tardaron **56,240 y 104,053 s**. Se retiró la ampliación de capturas con jaque que evitaba la poda delta, conservando la protección de ventanas de mate, y se añadió una comprobación barata de avance libre de peón para evitar generar todas las jugadas antes de devolver muchos cortes. Los PGN de los dos experimentos son `build/base-improvement-match.pgn` y `build/base-improvement-final-match.pgn`; ambos son artefactos locales, no la evidencia de la versión finalmente entregada. En Jugar sí cambia la selección del perfil: ChessBot pasa de `Baseline` a `Optimized` con Hash 256 MB; Stockfish conserva sus opciones. No se ha entrenado ni promovido una red nueva.

Una repetición de la versión anterior `56e8056` con el mismo benchmark produjo **96,035 s**, completando también las tres variantes a profundidad 20 (378.285.049 nodos). La fuerte variación respecto a las muestras históricas de 34–38 s impide atribuir todos los cambios de tiempo a una modificación concreta con tan pocas muestras. No se promete un tiempo fijo para profundidad 20; la interacción en directo permite inspeccionar y cambiar de prioridad mientras sigue la búsqueda.

La versión entregada completó profundidad 20 en las tres variantes con ocho hilos en **60,193 y 37,292 s**, con 239.192.887 y 150.419.638 nodos respectivamente. Cada muestra arrancó con motor frío y Hash 256 MB; el comando reproducible es `tools/benchmark_analysis.py --engine build/Release/chessbot.exe --depth 20 --multipv 3 --threads 8 --runs 2`. Estas dos muestras no bastan para estimar una aceleración estable. La base entregada conserva la detección de ahogado y las protecciones de ventanas de mate; la ampliación de capturas con jaque no forma parte de ella.

Su comprobación final de **32 partidas** contra `56e8056`, mismo protocolo de 50 ms/120 plies/colores intercambiados, obtuvo **4 victorias, 21 tablas y 7 derrotas (14,5–17,5)**. El intervalo aproximado de puntuación por parejas fue 40,4–50,3 %. Se conserva como corrección de búsqueda con regresión funcional demostrable, **no como una ganancia de Elo demostrada**; sigue siendo necesario ampliar la evaluación de fuerza y no se descarta una regresión. Artefacto local: `build/base-corrections-delivery-match.pgn`.

Verificación local final: CTest Release **3/3** (incluida PERFT completa), **46** casos C++, CLI, UCI, análisis por lotes y en directo, sesión PGN/FEN, **63** puntuaciones NNUE idénticas Python/C++, integración WinForms, Ruff y formato C++. El lanzador Release se copió al destino del acceso directo del escritorio, comprobando hashes de sus cinco artefactos. La prueba en directo se incorpora al workflow de Linux/Windows Release; los resultados locales no sustituyen una nueva ejecución remota de CI.

### Propuestas múltiples y planes

La ampliación del explicador reproduce la posición real `r1b2r2/bpp2pk1/p6p/4p1pQ/1P2P3/1BPq4/P2N1PPP/3R1RK1 b - - 1 16` y comprueba que una pregunta con `Qxc3` y `Qxf1` detecta ambas jugadas legales aunque solo una figure en el campo explícito. Las propuestas se buscan juntas con la mejor referencia como raíces de una misma MultiPV, evitando presentar como comparación directa puntuaciones de búsquedas separadas. Cada resultado conserva pieza movida/capturada, jaque, actores de cada ply y PV.

La regresión de sesión evalúa dos propuestas SAN en una llamada, exige la misma referencia para ambas y verifica que las secuencias distinguen negras y blancas. La integración WinForms introduce dos alternativas desde el escritorio y exige que ambas aparezcan en la explicación. El adaptador de Ollama usa una salida JSON estructurada para los planes; los veredictos numéricos se redactan de forma determinista antes del texto del modelo. Una comprobación local real con `qwen3.5:9b` devolvió secciones separadas para las propuestas y los planes de ambos bandos. Las ideas estratégicas continúan siendo interpretación condicionada por la PV, no hechos equivalentes a la evaluación del motor.
